...
WORKDIR /gopath/src/github.com/UstyuzhaninAV/clicktail
COPY . /gopath/src/github.com/UstyuzhaninAV/clicktail/
...
RUN apk add --update \
    ...
    && git clean -f \
    && rm -rf .git \
    && go get -ldflags="-X main.BuildID=${ver}" github.com/UstyuzhaninAV/clicktail \
    && apk del git go \
    && rm -rf /var/cache/apk/*
